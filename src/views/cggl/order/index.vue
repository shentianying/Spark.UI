<template>
  <div class="app-container">
    <search-form :show-toggle="true">
      <template #row1>
        <el-input v-model="search.number" placeholder="订单编号" size="mini" clearable maxlength="20" style="width:150px;" />
        <org-select2
    ...
    >
      <el-col style="text-align:left;">
        ...
        <el-button v-has="'Order_EDIT'" :loading="listLoading" type="primary" size="mini" icon="el-icon-circle-plus-outline" @click="handleAdd">添加</el-button>
      </el-col>
    ...
    >
      ...
      <el-table-column label="供应商" align="center" min-width="20">
        <template slot-scope="scope">
...
<script>
import { getOrderPaging } from '@/api/cggl/order'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getOrderPaging(postData).then(response => {
    ...
    handleAdd(row) {
      this.$router.push({ name: 'OrderEdit', query: { refreshRouterName: this.$route.name }, params: { type: 3 }})
    },
    handleEdit(row) {
      this.$router.push({ name: 'OrderEdit', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id }})
    },
    attitude(row) {
      this.$router.push({ name: 'OrderAttitude', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id }})
    },
    getFormState(row) {
