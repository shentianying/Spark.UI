<template>
  <div class="app-container">
    ...
    >
      <el-col style="text-align:left;">
        <el-form-item label="合同编号" prop="number">
          <el-input v-model="search.number" placeholder="合同编号" size="mini" clearable maxlength="20" style="width:150px;" @keyup.enter.native="fetchData" />
        </el-form-item>
        ...
        <el-form-item label="报价单号">
          <el-input v-model="search.quotationNumber" placeholder="请输入" size="mini" clearable maxlength="20" style="width:150px;" @keyup.enter.native="fetchData" />
        </el-form-item>
        ...
        <el-form-item label="材料名" prop="categoryName">
          <el-input v-model="search.categoryName" placeholder="请输入" size="mini" clearable maxlength="20" style="width:150px;" @keyup.enter.native="fetchData" />
        </el-form-item>
        <el-button type="primary" :loading="listLoading" icon="el-icon-search" size="mini" @click="fetchData">搜索</el-button>
      </el-col>
    ...
    >
      ...
      <el-table-column label="报价单号" header-align="center" width="130">
        <template slot-scope="scope">
      ...
      <el-table-column label="合同金额" header-align="center" align="right" min-width="15">
        <template slot-scope="scope">
          ￥{{ scope.row.contractAmount.toFixed(4) }}
        </template>
      </el-table-column>
      <el-table-column label="应付金额" header-align="center" align="right" min-width="15">
        <template slot-scope="scope">
          ￥{{ scope.row.dues.toFixed(4) }}
        </template>
      </el-table-column>
      <el-table-column label="已付金额" header-align="center" align="right" min-width="15">
        <template slot-scope="scope">
          ￥{{ scope.row.paid.toFixed(4) }}
        </template>
      </el-table-column>
      <el-table-column label="开票金额" header-align="center" align="right" min-width="15">
        <template slot-scope="scope">
          ￥{{ scope.row.receipt.toFixed(4) }}
        </template>
      </el-table-column>
      ...
      <!-- <el-table-column label="操作" align="center" width="80">
        <template slot-scope="scope">
          ...
          <el-tooltip content="查看" effect="light" placement="left-start" :enterable="false">
            <el-button v-if="scope.row.state>0" size="mini" class="el-icon-document" plain circle @click="detail(scope.row)" />
          </el-tooltip>
        </template>
      </el-table-column> -->
    </el-table>
...
<script>
import { getContractAmountPaging } from '@/api/cggl/contract'
...
export default {
  ...
  data() {
    return {
      ...
      tabPosition: 0, // 0:我发起，1：待审批 2:已审批
      supplierList: [],
      rules: {}
    }
  },
  ...
  methods: {
    fetchData() {
      ...
      getContractAmountPaging(postData).then(response => {
    ...
    // }
    detail(row) {
      this.$router.push({ name: 'ContractDetail', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id }})
    }
  }
}
</script>
