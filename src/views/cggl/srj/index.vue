...
<script>
import { getIncomeCostPaging } from '@/api/cggl/incomeCost'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getIncomeCostPaging(postData).then(response => {
    ...
    handleAdd(row) {
      this.$router.push({ name: 'IncomeCostEdit', query: { refreshRouterName: this.$route.name }})
    },
    handleEdit(row) {
      // console.log('this.$route.name:', this.$route.name)
      this.$router.push({ name: 'IncomeCostEdit', query: { id: row.id, refreshRouterName: this.$route.name, tab: 'edit' }, params: { id: row.id }})
    },
    attitude(row) {
      this.$router.push({ name: 'IncomeCostAttitude', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id }})
    },
    detail(row) {
      this.$router.push({ name: 'IncomeCostAttitude', query: { id: row.id, refreshRouterName: this.$route.name, state: row.state }, params: { id: row.id }})
    },
    sizeChange(val) {
      this.pageSize = val
      this.fetchData()
