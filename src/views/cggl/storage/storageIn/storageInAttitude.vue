<template>
  <div class="app-container">
      ...
      <el-row :gutter="10">
        <el-col :sm="12" :xs="24">
          ...
          <el-form-item label="供应商名称">
            <el-link v-if="ruleForm.supplier" :underline="false" @click="openSupplier(ruleForm.supplier.id)">{{ ruleForm.supplier.name }}</el-link>
          </el-form-item>
          <el-form-item label="入库时间" prop="entryDate">
            {{ ruleForm.entryDate | dateFormat }}
          </el-form-item>
    ...
    >
      ...
      <el-table-column label="含税单价" header-align="center" align="right" min-width="200">
        <template slot-scope="scope">
          {{ scope.row.taxInclusivePrice.toFixed(2) }}
        </template>
      </el-table-column>
      <el-table-column label="税率" header-align="center" align="right" min-width="200">
        <template slot-scope="scope">
          {{ scope.row.taxRate }}
        </template>
      </el-table-column>
      <el-table-column label="不含税单价" header-align="center" align="right" min-width="200">
        <template slot-scope="scope">
          {{ scope.row.taxExclusivePrice.toFixed(2) }}
        </template>
      </el-table-column>
      <el-table-column label="含税金额" header-align="center" align="right" min-width="200">
        <template slot-scope="scope">
          {{ scope.row.taxInclusiveAmount.toFixed(2) }}
        </template>
      </el-table-column>
      <el-table-column label="税额" header-align="center" align="right" min-width="200">
        <template slot-scope="scope">
          {{ scope.row.tax.toFixed(2) }}
        </template>
      </el-table-column>
      <el-table-column label="不含税金额" header-align="center" align="right" min-width="200">
        <template slot-scope="scope">
          {{ scope.row.taxExclusiveAmount.toFixed(2) }}
        </template>
      </el-table-column>
...
<script>
import { getStorageInById, addAttitude } from '@/api/cggl/storageIn'
...
export default {
  ...
  methods: {
    ...
    getTypeName(type) {
      if (type !== null) {
        var entryType = this.entryTypeOptions.find(f => f.value === type)
