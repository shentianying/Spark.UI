<template>
  <div class="app-container">
    <search-form :show-toggle="true">
      ...
      <template #row2>
        <el-select v-model="search.entryTypes" multiple collapse-tags style="width:150px" size="mini" placeholder="入库类型">
          <el-option v-for="(item,index) in entryTypeList" :key="index" :label="item.name" :value="item.value" />
        </el-select>
    ...
    >
      <el-col style="text-align:left;">
        ...
        <el-form-item label="入库类型">
          <el-select v-model="search.entryTypes" multiple collapse-tags style="width:150px" size="mini" placeholder="请选择">
            <el-option v-for="(item,index) in entryTypeList" :key="index" :label="item.name" :value="item.value" />
          </el-select>
        </el-form-item>
    ...
    <el-table
      ...
      size="mini"
    >
      <el-table-column label="入库单号" header-align="center" width="130">
      ...
      <el-table-column label="入库时间" header-align="center" width="130">
        <template slot-scope="scope">
          {{ scope.row.entryDate | dateFormat }}
        </template>
      </el-table-column>
      <el-table-column label="入库类别" align="center" width="130">
        <template slot-scope="scope">
...
<script>
import { getStorageInPaging } from '@/api/cggl/storageIn'
...
export default {
  ...
  data() {
    return {
      ...
      search: {
        ...
        orgIds: [],
        states: [],
        // entryDateStartDate: null,
        // entryDateEndDate: null,
        entryDateRange: [],
        entryTypes: [],
        supplierIds: [],
        personIds: [],
  ...
  methods: {
    ...
    fetchData() {
      ...
      // postData.entryDateStartDate = this.search.entryDateStartDate
      // postData.entryDateEndDate = this.search.entryDateEndDate
      postData.entryDateRange = this.search.entryDateRange
      postData.entryTypes = this.search.entryTypes
      ...
      getStorageInPaging(postData).then(response => {
    ...
    handleAdd() {
      this.$router.push({ name: 'StorageInEdit', query: { refreshRouterName: this.$route.name }, params: { type: 3 }})
    },
    handleEdit(row) {
      this.$router.push({ name: 'StorageInEdit', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id, type: row.type }})
    },
    attitude(row) {
    ...
    getTypeName(type) {
      return this.entryTypeList.find(f => f.value === type).name
    },
