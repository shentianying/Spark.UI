...
<script>
import { getAllocationById, addAttitude } from '@/api/cggl/allocation'
...
export default {
  ...
  methods: {
    ...
    setData(response) {
      const { data, orgStrOut, orgStrIn, billFlowData } = response
      this.ruleForm = data
      this.ruleForm.fileList = data.upFile
      this.orgStrOut = orgStrOut
      this.orgStrIn = orgStrIn
    ...
    getTypeName(type) {
      if (type !== null) {
        var entryType = this.entryTypeOptions.find(f => f.id === type)
