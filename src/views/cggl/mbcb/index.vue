...
<script>
import { getTargetCostPaging } from '@/api/cggl/targetCost'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getTargetCostPaging(postData).then(response => {
    ...
    handleEdit(row) {
      ...
      this.$router.push({ name: 'TargetCostEdit', query: { id: row.id, refreshRouterName: this.$route.name, tab: 'edit' }, params: { id: row.id }})
    },
    attitude(row) {
