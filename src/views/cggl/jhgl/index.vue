<template>
  <div class="app-container">
    <search-form :show-toggle="true">
      <template #row1>
        <el-input v-model="search.number" placeholder="计划编号" size="mini" clearable maxlength="20" style="width:150px;" />
        <org-select2
      ...
      <template #row2>
        <el-input v-model="search.realNumber" placeholder="实际计划单号" size="mini" clearable maxlength="20" style="width:150px;" />
        <el-select v-model="search.planTypes" multiple style="width:150px" size="mini" placeholder="计划类型">
          <el-option v-for="(item,index) in planTypeList" :key="index" :label="item.name" :value="item.value" />
        </el-select>
        <el-select v-model="search.materialTypes" collapse-tags clearable multiple style="width:150px" size="mini" placeholder="材料类型">
    ...
    >
      ...
      <el-table-column label="计划编号" header-align="center" width="130">
      ...
      <el-table-column label="材料类型" align="center" min-width="10">
        <template slot-scope="scope">
          {{ materialTypeList.find(f=>f.value === scope.row.materialType).name }}
        </template>
      </el-table-column>
...
<script>
import { getPlanPaging } from '@/api/cggl/plan'
import OrgSelect2 from '@/components/OrgSelect/index2'
import SearchForm from '@/components/Other/searchForm'
// import { LSGetObject } from '@/utils/localstore'
export default {
  name: 'Plan',
  components: {
    OrgSelect2,
  ...
  methods: {
    ...
    fetchData() {
      ...
      postData.planTypes = this.search.planTypes
      postData.materialTypes = this.search.materialTypes
      postData.categoryName = this.search.categoryName
      postData.orgIds = this.search.orgIds
      postData.realNumber = this.search.realNumber
      ...
      getPlanPaging(postData).then(response => {
        const { data, count, planTypeList, materialTypeList } = response
        this.list = data
        this.total = count
    ...
    handleAdd(row) {
      this.$router.push({ name: 'PlanEdit', query: { refreshRouterName: this.$route.name }, params: { type: 3 }})
    },
    handleEdit(row) {
      this.$router.push({ name: 'PlanEdit', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id }})
    },
    attitude(row) {
      this.$router.push({ name: 'PlanAttitude', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id }})
    },
    getFormState(row) {
