<template>
  <div class="app-container">
    <search-form :show-toggle="true">
      <template #row1>
        <el-input v-model="search.number" placeholder="送货单号" size="mini" clearable maxlength="20" style="width:150px;" />
        <org-select2
    ...
    >
      <el-col style="text-align:left;">
        <el-form-item label="送货单号" prop="number">
          <el-input v-model="search.number" placeholder="送货单号" size="mini" clearable maxlength="20" style="width:150px;" />
        </el-form-item>
    ...
    <el-table
      ...
      size="mini"
    >
      <!-- <el-table-column label="订单编号" header-align="center" width="130">
        <template slot-scope="scope">
          {{ scope.row.orderNumber }}
        </template>
      </el-table-column> -->
      <el-table-column label="送货单号" header-align="center" width="130">
        <template slot-scope="scope">
          {{ scope.row.number }}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="送货日期" min-width="20">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" width="80">
        <template slot-scope="scope">
          ...
          <el-tooltip content="详情" effect="light" placement="left-start" :enterable="false">
            <el-button v-if="scope.row.state>0" size="mini" class="el-icon-edit-outline" plain circle @click="detail(scope.row)" />
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { getDeliveryPaging } from '@/api/cggl/delivery'
...
export default {
  ...
  data() {
    return {
      ...
      search: {
        ...
        projectId: null,
        states: [],
        // deliveryDateStartDate: null,
        // deliveryDateEndDate: null,
        deliveryDateRange: [],
        supplierIds: [],
        categoryName: ''
      },
  ...
  methods: {
    ...
    fetchData() {
      ...
      // postData.deliveryDateStartDate = this.search.deliveryDateStartDate
      // postData.deliveryDateEndDate = this.search.deliveryDateEndDate
      postData.deliveryDateRange = this.search.deliveryDateRange
      postData.supplierIds = this.search.supplierIds
      postData.categoryName = this.search.categoryName
      getDeliveryPaging(postData).then(response => {
    ...
    handleAdd(row) {
      this.$router.push({ name: 'DeliveryEdit', query: { refreshRouterName: this.$route.name }, params: { type: 3 }})
    },
    handleEdit(row) {
      this.$router.push({ name: 'DeliveryEdit', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id }})
    },
    detail(row) {
      this.$router.push({ name: 'DeliveryDetail', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id }})
    },
    getFormState(row) {
      ...
      const state = row.state
      if (state === -1) return '作废'
      else if (state === 0) return '制单'
      else if (state === 1) return '已送货'
      else if (state === 2) return '已入库'
    },
    sizeChange(val) {
      this.pageSize = val
      this.fetchData()
