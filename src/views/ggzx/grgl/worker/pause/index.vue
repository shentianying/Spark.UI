<template>
  <div class="app-container workerPause">
    <SearchBar ref="SearchBar" v-model="searchData" :size="size" :search="handelSearch" />
    <div class="action">
      <div>
        <el-button v-has="'GRZT_ADD'" :size="size" icon="el-icon-plus" type="primary" @click="handleEdit('')">新建</el-button>
      </div>
    ...
    >
      ...
      <el-table-column align="center" label="暂退原因" min-width="10">
        <template slot-scope="scope">
          {{ scope.row.pauseReason }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="暂退日期" min-width="10">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" width="120">
        <template v-slot="{ row }">
          <el-button v-if="row.state===0" v-has="'GRZT_EDIT'" :size="size" type="text" @click="handleEdit(row.id)">编辑</el-button>
          <el-button v-if="row.state>0&&row.state<10000" v-has="'GRZT_ATTITUDE'" :size="size" type="text" @click="attitude(row.id)">审批</el-button>
          <el-button v-if="row.state===10000" v-has="'GRZT_ATTITUDE'" :size="size" type="text" @click="attitude(row.id)">审批详细</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="page">
...
<script>
import { getWorkerPausePaging } from '@/api/ggzx/workerPause'
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        // 请求开始
        const response = await getWorkerPausePaging({
          ...postData,
          currentPage,
          pageSize,
     ...
     */
    handleDetails(row) {
      this.$router.push({ name: 'WorkerPauseDetail', query: { id: row.id, refreshRouterName: this.$route.name }})
    },
    /**
     * @description: table编辑
     * @param {Object} id
     */
    handleEdit(id) {
      this.$router.push({ name: 'WorkerPauseEdit', query: { id, refreshRouterName: this.$route.name }})
    },
    getFormState(row) {
    ...
    // 审批
    attitude(id) {
      this.$router.push({ name: 'WorkerPauseAttitude', query: { id, refreshRouterName: this.$route.name }})
    }
  }
}
</script>
