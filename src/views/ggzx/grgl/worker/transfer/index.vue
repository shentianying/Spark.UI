<template>
  <div class="app-container workerExit">
    <SearchBar ref="SearchBar" v-model="searchData" :size="size" :search="handelSearch" />
    <div class="action">
      <div>
        <el-button v-has="'GRDD_ADD'" :size="size" icon="el-icon-plus" type="primary" @click="handleEdit('')">新建</el-button>
      </div>
    ...
    >
      <el-table-column align="center" label="编号" min-width="40">
        <template slot-scope="scope">
          <el-link :underline="false" style="font-size: 12px;" @click="handleDetails(scope.row)">{{ scope.row.number }}</el-link>
          <!-- {{ scope.row.number }} -->
        </template>
      </el-table-column>
      <el-table-column align="center" label="调出部门" min-width="20">
        <template slot-scope="scope">
          {{ scope.row.organizationIn.name }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="调入部门" min-width="20">
        <template slot-scope="scope">
          {{ scope.row.organizationOut.name }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="工号" min-width="10">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" width="120">
        <template v-slot="{ row }">
          <el-button v-if="row.state===0" v-has="'GRDD_EDIT'" :size="size" type="text" @click="handleEdit(row.id)">编辑</el-button>
          <el-button v-if="row.state>0&&row.state<10000" v-has="'GRDD_ATTITUDE'" :size="size" type="text" @click="attitude(row.id)">审批</el-button>
          <el-button v-if="row.state===10000" v-has="'GRDD_ATTITUDE'" :size="size" type="text" @click="attitude(row.id)">审批详细</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="page">
...
<script>
import { getWorkerTransferPaging } from '@/api/ggzx/workerTransfer'
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        // 请求开始
        const response = await getWorkerTransferPaging({
          ...postData,
          currentPage,
          pageSize,
     ...
     */
    handleDetails(row) {
      this.$router.push({ name: 'WorkerTransferDetail', query: { id: row.id, refreshRouterName: this.$route.name }})
    },
    /**
     * @description: table编辑
     * @param {Object} id
     */
    handleEdit(id) {
      this.$router.push({ name: 'WorkerTransferEdit', query: { id, refreshRouterName: this.$route.name }})
    },
    getFormState(row) {
    ...
    // 审批
    attitude(id) {
      this.$router.push({ name: 'WorkerTransferAttitude', query: { id, refreshRouterName: this.$route.name }})
    }
  }
}
</script>
