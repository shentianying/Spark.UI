<template>
  <div class="app-container workerExit">
    ...
    <div class="action">
      <div>
        <el-button v-has="'GRTC_ADD'" :size="size" icon="el-icon-plus" type="primary" @click="handleEdit('')">新建</el-button>
      </div>
    ...
    >
      ...
      <el-table-column align="center" label="退场原因" min-width="10">
        <template slot-scope="scope">
          {{ scope.row.exitReason }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="退场日期" min-width="10">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" width="120">
        <template v-slot="{ row }">
          <el-button v-if="row.state===0" v-has="'GRTC_EDIT'" :size="size" type="text" @click="handleEdit(row.id)">编辑</el-button>
          <el-button v-if="row.state>0&&row.state<10000" v-has="'GRTC_ATTITUDE'" :size="size" type="text" @click="attitude(row.id)">审批</el-button>
...
<script>
import { getWorkerExitPaging } from '@/api/ggzx/workerExit'
...
export default {
  ...
  methods: {
    ...
    async fetchData(type) {
      ...
      try {
        // 获取请求参数
        const postData = this.getSearchData(type)
        const { currentPage, pageSize } = this.table
        // 请求开始
        const response = await getWorkerExitPaging({
          ...postData,
          currentPage,
          pageSize,
     ...
     */
    handleDetails(row) {
      this.$router.push({ name: 'WorkerExitDetail', query: { id: row.id, refreshRouterName: this.$route.name }})
    },
    /**
     * @description: table编辑
     * @param {Object} id
     */
    handleEdit(id) {
      this.$router.push({ name: 'WorkerExitEdit', query: { id, refreshRouterName: this.$route.name }})
    },
    getFormState(row) {
    ...
    // 审批
    attitude(id) {
      this.$router.push({ name: 'WorkerExitAttitude', query: { id, refreshRouterName: this.$route.name }})
    }
  }
}
</script>
