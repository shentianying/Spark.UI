<template>
  <div class="app-container">
    <div v-show="active===0">
      ...
      >
        ...
        <el-table-column label="是否办理工伤保险" align="center" width="200">
          <template slot-scope="scope">
            <el-switch
              v-model="scope.row.worker.isWithInsurance"
              active-color="#13ce66"
              inactive-color="#ff4949"
            />
            <!-- {{ scope.row.worker.isWithInsurance?'已办理':'未办理' }} -->
          </template>
        </el-table-column>
        <el-table-column label="工伤保险开始日期" align="center" width="200">
          <template slot-scope="scope">
            <el-date-picker
              v-model="scope.row.worker.insuranceDate"
              type="date"
              value-format="yyyy-MM-dd"
              placeholder="请选择"
              style="width:100%"
    ...
    <select-worker-2 ref="workerRef" :multiple="true" :org-ids="orgIdList" :bd-state="false" :worker-list="workerList" @getPersons="getPersons" />
  </div>
</template>
<script>
import { getFormById, getBillFlow, saveData, deleteData } from '@/api/ggzx/workerRegister'
        ...
        if (valid) {
          ...
          this.list.forEach(e => {
            details.push({
              ...
              isWithInsurance: e.worker.isWithInsurance,
              insuranceDate: e.worker.insuranceDate,
              remark: e.remark
            })
          })
          ...
          const postdata = {
            id: newObj.id,
            orgId: newObj.orgId,
            registerDate: newObj.registerDate,
            remark: newObj.remark,
            fileList: newObj.fileList,
            state: newObj.state,
            workerRegisterDetails: details,
            billFlow: billFlow
          }
        ...
        personList.forEach(person => {
          data.push({
            worker: {
              ...
              isWithInsurance: person.isWithInsurance,
              insuranceDate: person.insuranceDate
            },
            registerDate: this.ruleForm.registerDate,
            remark: ''
          })
          workers.push(person)
        })
