...
import { getMatchPaging } from '@/api/hdgl/match.js'
...
export default {
  ...
  methods: {
    ...
    parseServeDate(res) {
      ...
      this.$refs['SearchBar'].setData('types', 'options', matchTypeList, 'init')
    ...
    getTable(currentPage = this.table.currentPage, pageSize = this.table.pageSize) {
      ...
      // 请求开始
      return getMatchPaging(postData)
    },
     ...
     */
    handleDetails(row) {
      this.$router.push({ name: 'MatchDetail', query: { id: row.id, refreshRouterName: this.$route.name }})
    },
    /**
     * @description: table编辑
     * @param {Object} row
     */
    handleEdit(id = '', type) {
      this.$router.push({ name: 'MatchEdit', query: { type, id, refreshRouterName: this.$route.name }})
    }
  }
}
</script>
