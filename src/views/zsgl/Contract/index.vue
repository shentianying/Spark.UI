...
import { getContractPaging } from '@/api/zsgl/contract.js'
...
export default {
      ...
      table: {
        ...
        columns: [
          ...
          },
          {
            ...
            prop: 'state',
            width: '120',
            format: (val) => val === 0 ? '未生效' : val === 1 ? '已生效' : '已失效'
          },
          {
            label: '是否永久合同',
            prop: 'isForever',
  ...
  methods: {
    ...
    getTable(currentPage = this.table.currentPage, pageSize = this.table.pageSize) {
      ...
      // 请求开始
      return getContractPaging(postData)
    },
     ...
     */
    handleDetails(row) {
      this.$router.push({ name: 'ContractDetails', query: { id: row.id, refreshRouterName: this.$route.name }})
    },
    /**
     * @description: table编辑
     * @param {Object} row
     */
    handleEdit(id = '', type) {
      this.$router.push({ name: 'ContractEdit', query: { type, id, refreshRouterName: this.$route.name }})
    }
  }
}
</script>
