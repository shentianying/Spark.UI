  ...
  <div class="app-container shouKuan">
    ...
    >
      <template v-slot:actions="{row}">
        ...
        <el-button v-else-if="row.state === 1000 && utils.IsNextPerson(row.billFlow)" :size="size" type="text" @click="handleDetails(row,'pay')">支付</el-button>
...
import { GetInvoiceReimbursePaging } from '@/api/cwgl/InvoiceReimburse.js'
import SearchBar from '@/components/SearchBar'

export default {
  name: 'InvoiceReimburseIndex',
  components: {
    SearchBar,
    TableEx
  },
  ...
  methods: {
    ...
    parseServeDate(res) {
      ...
      const { paymentStateList } = this.filter
      this.$refs['SearchBar'].setData('states', 'options', data[0]?.billFlow?.form?.formState, 'init')
      this.$refs['SearchBar'].setData('invoiceTypes', 'options', invoiceTypeList, 'init')
      this.$refs['SearchBar'].setData('reimburseType', 'options', reimburseTypeList, 'init')
      this.$refs['SearchBar'].setData('paymentState', 'options', paymentStateList, 'init')
      // 列表数据初始化
      this.amount = amount
    ...
    getTable(currentPage = this.table.currentPage, pageSize = this.table.pageSize) {
      ...
      // 请求开始
      return GetInvoiceReimbursePaging(postData)
    },
    ...
    handleDetails(row, type = 'details') {
      this.$router.push({ name: 'InvoiceReimburseDetails', query: { type, id: row.id, refreshRouterName: this.$route.name }})
    },
    /**
     * @description: table编辑
     * @param {Object} row
     */
    handleEdit(row, type) {
      this.$router.push({ name: 'InvoiceReimburseEdit', query: { type, id: row?.id, refreshRouterName: this.$route.name }})
    }
  }
}
</script>
