...
import { GetImprestAppliactionPaging } from '@/api/cwgl/Impresst.js'
import SearchBar from '@/components/SearchBar'
// //import searchData from './searchData'

export default {
  name: 'ImpresstApplicationIndex',
  components: {
    SearchBar,
    TableEx
  },
  data() {
    return {
      ...
      searchData: require('./ApplicationIndexSearchData').default(), // 渲染搜索栏数据
  ...
  methods: {
    ...
    getTable(currentPage = this.table.currentPage, pageSize = this.table.pageSize) {
      ...
      // 请求开始
      return GetImprestAppliactionPaging(postData)
    },
    ...
    handleDetails(row, type = 'details') {
      this.$router.push({ name: 'ImprestApplicationDetails', query: { type, id: row.id, refreshRouterName: this.$route.name }})
    },
    /**
     * @description: table编辑
     * @param {Object} row
     */
    handleEdit(id = '', type) {
      this.$router.push({ name: 'ImprestApplicationEdit', query: { type, id, refreshRouterName: this.$route.name }})
    }
  }
}
</script>
