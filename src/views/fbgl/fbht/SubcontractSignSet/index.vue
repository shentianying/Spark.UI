  ...
  <div class="app-container shouKuan">
    ...
    >
      <template v-slot:actions="{row}">
        ...
        <el-button v-else-if="row.state !== 10000 && utils.IsNextPerson(row.billFlow)" :size="size" type="text" @click="handleDetails(row,'approval')">审批</el-button>
...
import { GetSubcontractSignSetPaging } from '@/api/fbgl/subcontractSignSet.js'
import SearchBar from '@/components/SearchBar'
// //import searchData from './searchData'

export default {
  name: 'SubcontractSignSetIndex',
  components: {
    SearchBar,
    TableEx
  },
  ...
  methods: {
    ...
    getTable(currentPage = this.table.currentPage, pageSize = this.table.pageSize) {
      ...
      // 请求开始
      return GetSubcontractSignSetPaging(postData)
    },
    ...
    handleDetails(row, type = 'details') {
      this.$router.push({ name: 'SubcontractSignSetDetails', query: { type, id: row.id, refreshRouterName: this.$route.name }})
    },
    /**
     * @description: table编辑
     * @param {Object} row
     */
    handleEdit(id = '', type) {
      this.$router.push({ name: 'SubcontractSignSetEdit', query: { type, id, refreshRouterName: this.$route.name }})
    }
  }
}
</script>
