...
import { GetProjectDurationPaging } from '@/api/xmgl/ProjectDuration.js'
import SearchBar from '@/components/SearchBar'
// //import searchData from './searchData'

export default {
  name: 'ProjectDurationIndex',
  components: {
    SearchBar,
    TableEx
  },
  ...
  methods: {
    ...
    parseServeDate(res) {
      ...
      this.$refs['SearchBar'].setData('states', 'projectState', [...projectStateList, { name: '全部', value: '' }], 'init')
    ...
    getTable(currentPage = this.table.currentPage, pageSize = this.table.pageSize) {
      ...
      // 请求开始
      return GetProjectDurationPaging(postData)
    },
     ...
     */
    handleDetails(row) {
      this.$router.push({ name: 'ProjectDurationDetails', query: { id: row.id, refreshRouterName: this.$route.name }})
    },
    /**
     * @description: table编辑
     * @param {Object} row
     */
    handleEdit(id = '', type) {
      this.$router.push({ name: 'ProjectDurationEdit', query: { type, id, refreshRouterName: this.$route.name }})
    }
  }
}
</script>
