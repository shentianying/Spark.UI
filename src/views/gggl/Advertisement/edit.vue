...
import { getAdvertisementById, initAdvertisement, addAdvertisement, putAdvertisement, deleteAdvertisement } from '@/api/gggl/advertisement.js'
export default {
  ...
  computed: {
    title() {
      const { active } = this
      return active === 2 ? '审批流程' : '广告管理'
    },
    formNodeData() {
      const {
        inputPlaceholder,
        selectPlaceholder,
        filter: { advertisementTypeList }
      } = this

      return [
        { label: '主题:', keyword: 'title', props: { placeholder: inputPlaceholder }},
        { label: '类型:', keyword: 'type', component: 'Select', props: { propsLabel: 'name', placeholder: selectPlaceholder, options: advertisementTypeList }},
  ...
  methods: {
     ...
     */
    initSelect() {
      initAdvertisement().then(res => {
        this.filter.advertisementTypeList = res.advertisementTypeList
      })
    },
    /**
    ...
    invalidForm() {
      this.$confirm('作废以后将无法编辑，是否确定作废？', '提示', {
        ...
        return deleteAdvertisement({ id: this.firstForm.id })
      }).then(() => {
    ...
    submitForm() {
      ...
      if (req.id) {
        putAdvertisement(req).then(res => {
          // 更新
          this.$message.success('保存成功')
          this.back()
        })
      } else {
        addAdvertisement(req).then(() => {
          // 添加
