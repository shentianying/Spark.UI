<template>
  <div class="app-container list">
    ...
    <div class="action">
      <div>
        <el-button v-has="'INCOMECONTRACT_ADD'" :size="size" icon="el-icon-plus" type="primary" @click="handleAdd">新建</el-button>
      </div>
    ...
    >
      <template v-slot:actions="{row}">
        <el-button v-if="row.state === 0" v-has="'INCOMECONTRACT_EDIT'" :size="size" type="text" @click="handleEdit(row)">编辑</el-button>
        <el-button v-else-if="row.state === 1 && utils.IsNextPerson(row.billFlow)" v-has="'INCOMECONTRACT_ATTITUDE'" :size="size" type="text" @click="attitude(row)">审批</el-button>
        <el-button v-else-if="row.state === 100 && utils.IsNextPerson(row.billFlow)" v-has="'INCOMECONTRACT_ATTITUDE'" :size="size" type="text" @click="attitude(row)">盖章</el-button>
        <el-button v-else-if="row.state === 1000 && utils.IsNextPerson(row.billFlow)" v-has="'INCOMECONTRACT_ATTITUDE'" :size="size" type="text" @click="attitude(row)">备案</el-button>
        <el-button v-else v-has="'INCOMECONTRACT_DETAIL'" :size="size" type="text" @click="handleDetail(row.id)">查看</el-button>
      </template>
    </TableEx>
...
<script>
import { getIncomeContractPaging } from '@/api/ywgl/incomeContract'
...
export default {
  ...
  methods: {
    ...
    parseServeDate(res) {
      ...
      this.$refs['SearchBar'].setData('sourceType', 'options', sourceTypeList, 'init')
    ...
    getTable(currentPage = this.table.currentPage, pageSize = this.table.pageSize) {
      ...
      // 请求开始
      return getIncomeContractPaging(postData)
    },
    /**
     * @description: 搜索栏数据转换
    ...
    handleAdd() {
      this.$router.push({ name: 'IncomeContractEdit', query: { refreshRouterName: this.$route.name }})
    },
    handleEdit(row) {
      this.$router.push({ name: 'IncomeContractEdit', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id, type: 3 }})
    },
    attitude(row) {
      this.$router.push({ name: 'IncomeContractAttitude', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id }})
    },
    handleDetail(id) {
      this.$router.push({ name: 'IncomeContractDetail', query: { id: id, refreshRouterName: this.$route.name }, params: { id: id }})
    }
  }
}
</script>
