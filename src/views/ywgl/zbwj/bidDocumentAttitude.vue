      ...
      <el-row :gutter="10">
        <el-col :sm="12" :xs="24">
          <el-form-item label="投标开始日期：" prop="tenderStartDate">
            {{ (ruleForm.tenderStartDate || '').split(' ')[0] }}
          </el-form-item>
        </el-col>
        <el-col :sm="12" :xs="24">
      ...
      <el-row :gutter="10">
        <el-col :sm="12" :xs="24">
          <el-form-item label="投标保证金金额：" prop="bidDeposit">
            {{ ruleForm.bidDeposit }}万元
          </el-form-item>
        </el-col>
        <el-col :sm="12" :xs="24">
          <el-form-item label="保证金缴纳截止时间：" prop="depositPayEndDate">
            {{ (ruleForm.depositPayEndDate || '').split(' ')[0] }}
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row :gutter="10">
        <el-col :sm="24" :xs="24">
          <el-form-item label="履约担保方式：" prop="performanceGuaranteeWay">
            {{ ruleForm.performanceGuaranteeWay }}
          </el-form-item>
        </el-col>
      </el-row>
...
<script>
import { getBidDocumentAttitudeById, checkBidDocument } from '@/api/ywgl/bidDocument'
import FlowDisplay from '@/components/Flow/flowDisplay'
import FlowAttitude from '@/components/Flow/flowAttitude'
import UpFile from '@/components/UpLoad/UpFile'
export default {
  name: 'BidDocumentAttitude',
  components: {
    FlowAttitude,
    FlowDisplay,
    UpFile
  ...
  methods: {
    async initData() {
      ...
      if (this.$route.query.id) {
        this.bidDocument = this.$route.query.id
      }
      if (this.bidDocument) {
        this.loading = true
        await getBidDocumentAttitudeById({ id: this.bidDocument }).then(response => this.setData(response))
      }
    },
    setData(response) {
      // 验证
      ...
      const { data, billFlowData, orgStr, result1, result2, result3, result4 } = response
      this.orgStr = orgStr
    ...
    updateFlow() {
      this.loading = true
      const data = { 'id': this.bidDocument, 'attitude': this.$refs.flowAttitude.getPostData() }
      checkBidDocument(data).then(response => {
        if (response.success) {
