<template>
  <div class="app-container list">
    ...
    <div class="action">
      <div>
        <el-button v-has="'BUSSINESSBID_ADD'" :size="size" icon="el-icon-plus" type="primary" @click="handleAdd">新建</el-button>
      </div>
    ...
    >
      <template v-slot:actions="{row}">
        <el-button v-if="row.state === 0" v-has="'BUSSINESSBID_EDIT'" :size="size" type="text" @click="handleEdit(row)">编辑</el-button>
        <el-button v-else-if="row.state === 1 && utils.IsNextPerson(row.billFlow)" v-has="'BUSSINESSBID_ATTITUDE'" :size="size" type="text" @click="attitude(row)">审批</el-button>
        <el-button v-else v-has="'BUSSINESSBID_DETAIL'" :size="size" type="text" @click="handleDetail(row.id)">查看</el-button>
      </template>
    </TableEx>
...
<script>
import { getBussinessBidPaging } from '@/api/ywgl/bussinessBid'
...
export default {
  ...
  methods: {
    ...
    parseServeDate(res) {
      ...
      this.$refs['SearchBar'].setData('offerType', 'options', offerTypeList, 'init')
    ...
    getTable(currentPage = this.table.currentPage, pageSize = this.table.pageSize) {
      ...
      // 请求开始
      return getBussinessBidPaging(postData)
    },
    /**
     * @description: 搜索栏数据转换
    ...
    handleAdd() {
      this.$router.push({ name: 'BussinessBidEdit', query: { refreshRouterName: this.$route.name }})
    },
    handleEdit(row) {
      this.$router.push({ name: 'BussinessBidEdit', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id, type: 3 }})
    },
    attitude(row) {
      this.$router.push({ name: 'BussinessBidAttitude', query: { id: row.id, refreshRouterName: this.$route.name }, params: { id: row.id }})
    },
    handleDetail(id) {
