    ...
    <div>
      ...
      >
        ...
        <el-table-column label="预估得分" align="center" min-width="100" prop="branchEstimatedScore">
          <template slot-scope="scope">
            <el-input-number v-if="ruleForm.state === 1" v-model="scope.row.branchEstimatedScore" size="mini" controls-position="right" :precision="2" />
            <span v-else>{{ scope.row.branchEstimatedScore }}</span>
          </template>
        </el-table-column>
...
<script>
import { getCreditBidAttitudeById, checkCreditBid } from '@/api/ywgl/creditBid'
import FlowDisplay from '@/components/Flow/flowDisplay'
import FlowAttitude from '@/components/Flow/flowAttitude'
import UpFile from '@/components/UpLoad/UpFile'
export default {
  name: 'CreditBidAttitude',
  components: {
    FlowAttitude,
    FlowDisplay,
    UpFile
  ...
  methods: {
    ...
    updateFlow() {
      ...
      this.loading = true
      this.billFlow = null
      const data = { 'creditBid': newObj, 'attitude': this.$refs.flowAttitude.getPostData() }
      checkCreditBid(data).then(response => {
        if (response.success) {
