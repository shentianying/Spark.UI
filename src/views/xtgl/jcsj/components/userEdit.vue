<template>
  <div class="app-container">
    ...
    >
        ...
        <el-col :sm="12" :xs="24">
          <el-form-item label="部门" prop="organizationId">
            <org-select2
              ...
              @change="orgChange"
            />
            <!-- <treeselect v-model="ruleForm.organizationId" placeholder="请选择" :multiple="false" :default-expand-level="1" :flat="false" :options="treeList" /> -->
          </el-form-item>
        </el-col>
        <el-col :sm="12" :xs="24">
      ...
      <!-- <el-form-item label="用户数据权限" prop="userOrg">
        <treeselect
          v-model="userOrg"
          placeholder="请选择"
          :multiple="true"
      ...
      <!-- <el-row :gutter="10">
        ...
        <el-col>
          <el-form-item label="用户公司" prop="userOrg">
            <treeselect
              v-model="userOrg"
              placeholder="请选择"
              :multiple="true"
    ...
    <!-- <select-user-organization ref="child" /> -->
    <div style="text-align:center;margin-top:20px;">
      ...
      <el-button v-if="personId>0" size="mini" type="success" @click="editPersonPost()">编辑岗位信息➜</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        postId: [{ required: true, message: '不能为空', trigger: 'change' }],
  ...
  methods: {
    ...
    async fetchData() {
      if (this.$route.query.id !== 0) {
        await getPersonById({ id: this.$route.query.id }).then((response) => {
          const { data, data1, data5 } = response
          this.postList = data5
          this.ruleForm = data
          this.userOrg = data1
    ...
    editPersonPost() {
      this.closeThePage()
      this.$router.push({ name: 'PersonPostEdit', query: { id: this.personId, refreshRouterName: this.$route.name }, params: { id: this.personId }})
    }
  }
}
</script>
