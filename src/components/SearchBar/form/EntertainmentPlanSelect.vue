<template>
  <div class="PersonSelect2">
    <selectEntertainmentPlan ref="planRef" v-model="inputVal" :multiple="multiple" :max-item="maxItem" @getPlants="getPlants" />
    <el-select
      ref="elSelect"
      v-model="inputVal"
      :disabled="disabled"
...
import selectEntertainmentPlan from '@/components/Other/fpgl/selectEntertainmentPlan'
export default {
  name: 'EntertainmentPlanSelect',
  components: { selectEntertainmentPlan },
  ...
  methods: {
    getPlants(plantsList) {
      plantsList = plantsList || []
      if (this.multiple) {
        this.inputVal = [...this.inputVal, ...plantsList.map(item => item.id)]
      } else {
        this.inputVal = plantsList[0].id
      }
    },
    selectPlan() {
    ...
    async getPlantList() {
      try {
        this.loading = true
        const res = await GetEntertainmentPlanPaging({ currentPage: 1, pageSize: this.utils.getPersonalSettings().pageSize, states: [10000] })
