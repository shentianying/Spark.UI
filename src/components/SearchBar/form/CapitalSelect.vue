 ...
 */
import { GetGatheringPaging } from '@/api/cwgl/zjgl/Gathering.js'
// import { InitiFundAllocation } from '@/api/cwgl/zjgl/FundAllocation.js'
export default {
  name: 'CapitalSelect',
  components: {
  },
  ...
  computed: {
    myOptions() {
      const options = JSON.parse(JSON.stringify(this.options || this.remoteOptions))
      this.transfOptions(options)
      return options
    },
    modelValue: {
      get() {
        // console.log(this.selectValue, this.findAllNodeParents(this.selectValue, this.myOptions))
  ...
  created() {
    ...
    GetGatheringPaging({ currentPage: 1, pageSize: 1 }).then(res => {
      this.remoteOptions = res.capitalTypeOptions
    })
  },
  methods: {
    /**
