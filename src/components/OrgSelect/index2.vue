...
export default {
  ...
  props: {
    ...
    selectData: { type: [Array, Number], default: null }, // 初始化选中的数据
    // 组件参数
  ...
  created() {
    ...
    // console.log('传过来的选中的值：', this.selectData)
  },
  mounted() {
    if (this.options) {
      ...
      if (this.selectData) {
        this.modelValue = this.filterParentNodes(this.selectData)
        // console.log('解析出来的值', this.modelValue)
      }
    } else {
      this.fetchData()
    }
  ...
  methods: {
    ...
    fetchData() {
      getCurrentUserOrg().then(response => {
        ...
        if (!data) return
        if (this.dataMode === 0) {
          this.list = toTree(currentOrgList)
    ...
    processData(userOrgList, currentOrgList) {
      currentOrgList.forEach(e => {
        e.disabled = !userOrgList.find(f => f.id === e.id)
      })
