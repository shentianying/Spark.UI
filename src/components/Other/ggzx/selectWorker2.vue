<template>
  ...
  <div v-if="open">
    ...
    >
      ...
      >
        <el-table-column type="selection" :reserve-selection="true" fixed="left" align="center" width="55" />
...
export default {
  ...
  props: {
    ...
    workerList: { type: Array, default: () => [] }, // 默认选中工人组
    ...
    ispause: { type: Boolean, default: null },
    isexit: { type: Boolean, default: null }
  },
  data() {
    return {
      open: false,
  ...
  methods: {
    ...
    getPersonList(postData) {
      getWorkerList({
        ...
        this.tableList = data
        data.forEach(e => {
          var row = this.workerList.find(f => f.id === e.id)
          if (row) {
