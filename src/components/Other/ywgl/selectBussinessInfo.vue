      ...
      <el-container>
        ...
        <el-main style="padding-top:0px;padding-right:0px;">
          ...
          >
            ...
            <el-table-column align="center" label="立项情况" min-width="50">
              <template slot-scope="scope">
                <span v-if="scope.row.state === 10000">已立项</span>
                <span v-else>未立项</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleNodeClick(data, node) {
      ...
      this.searchText = ''
      this.getBussinessInfoList({ orgId: data.id })
    },
    tagClose(uid) {
      this.selectionItem = this.selectionItem.filter(f => f.id !== uid)
    },
    getBussinessInfoList(postData) {
      postData.state = this.state
      postData.formState = this.formState
    ...
    search() {
      ...
      this.buttonLoading = true
      setTimeout(() => {
        this.getBussinessInfoList(postData)
      }, 700)
    },
    save() {
      this.buttonLoading = true
      // 触发父组件的事件
      setTimeout(() => {
        this.$emit('getBussinessInfo', this.selectionItem)
        this.buttonLoading = false
        this.open = false
      }, 700)
    }
